...
bind_test_tbl <- mutate(bind_test_tbl,ActivityLabel = activity(Activity))
bind_train_tbl <- mutate(bind_train_tbl,ActivityLabel = activity(Activity))
...
bind_test_tbl <- select(bind_test_tbl,Subject,ActivityLabel,contains("mean"),contains("std"))
bind_train_tbl <- select(bind_train_tbl,Subject,ActivityLabel,contains("mean"),contains("std"))
...
avg_test <- bind_test_tbl %>%
  group_by(Subject,ActivityLabel) %>%
  summarise_all(mean)
avg_train <- bind_train_tbl %>%
  group_by(Subject,ActivityLabel) %>%
  summarise_all(mean)
